...
class CommaInsertedLexer(object):
    ...
    def pop(self):
        if self._saved:
            ...
            self._saved = None
            return out
        (_type,value) = self._raw.pop()
        if self._last != None:
            if _type == self._last and _type in self.comma:
                self._saved = (_type,value)
                return ("CHAR",",")
        self._last = _type
        return (_type,value)
...
class BufferingLexer(object):
    def __init__(self,_input):
        self._lexer = CommaInsertedLexer(_input)
        self._saved = None
    ...
    def peek_is(self,_type,value = None):
        (t,v) = self.peek()
        return t == _type and (value is None or v == value)

    def attempt(self,_type,value):
        p = self.peek_is(_type,value)
        if p:
            self.pop()
        return p
